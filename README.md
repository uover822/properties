...
[![Build Status](https://travis-ci.org/msr/properties.svg?branch=master)](https://travis-ci.org/msr/properties)
...
__This microservice provides the properties module data functionality.__
...
|--|--|--
|`role:properties,cmd:get (SC)` |Get properties data about a module|
...
![properties](properties.png?raw=true "properties")
...
and then run `properties` inside the repository folder to install its dependencies:
...
* [`properties-dev.js`](srv/properties-dev.js) : run the development configuration 
  with hard-coded network ports.

  ```sh
  $ node srv/properties-dev.js
  ```
  ...
  script [`properties-dev-mesh.js`](srv/properties-dev-mesh.js). For more on
  this, see the [msr-repl](//github.com/msr/msr-repl)
  repository.

* [`properties-stage.js`](srv/properties-stage.js) : run the staging
  ...
  ```sh
  $ node srv/properties-stage.js [PORT]
  ```

* [`properties-prod.js`](srv/properties-prod.js) : run the production
  ...
  ```sh
  $ BASES=x.x.x.x:port node srv/properties-prod.js
  ```
